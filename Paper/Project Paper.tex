...
>>>>>>> d8be519d937b71a6f46f6169dc1a9b331bcbc625
...
\par In this section we go over the three models we built in detail.
>>>>>>> d8be519d937b71a6f46f6169dc1a9b331bcbc625
...
\par See \ref{tab:param_index} for an explanation of each term. Thus the sugar uptake climbs quickly when below half saturation, and then grows more slowly past that point.
...
See \ref{tab:param_index} for an explanation of each term.  
...
\subsection*{Compartmental Model with Monod Equation}
We similarly fit the compartmental to the data \cite{mechanistic}. We were able to assimilate the experimental data with the model to solve for the coefficients used in the model. Using Scipy, we were able to solve that 
\[ \mu_{\text{max}} = 0.1685 \quad K_S = 0.0996 \quad Y_{X/S} = 0.1768 \quad Y_{P/S} = 0.7015. \] 
The model with these coefficients and initial values taken from the data is plotted alongside the data in \ref{fig:images/monod_modeling.png}
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.5]{images/monod_modeling.png}
\end{center}
\caption{The fitted categorical model plotted alongside the experimental data. \cite{mechanistic}}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.45]{mcrm.png}
