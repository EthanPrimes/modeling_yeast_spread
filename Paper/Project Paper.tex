...
\begin{align*}
    ...
    \dot{P} &= Y_{P/S} (-\dot{S}),
\end{align*}
where $Y_{X/S}$ denotes the grams of biomass of yeast per gram of sugar, and $Y_{P/S}$ denotes the product yield of yeast per gram of sugar. The plot of the solution is given in Figure \ref{fig:monod_categorical}.
...
\subsection*{Yield}
\par Additionally, the biomass that sugar provides yeast varies with the amount of oxygen available. Our model should reflect both aerobic and anaerobic metabolism of sugar. We want to model a smooth switch between aerobic and anaerobic metabolism. We accomplish this via a switching function, which we name $f$ in our model:
$$f(O) = \frac{O}{K_O + O}.$$
...
\par Thus, when $O \gg K_O$, $f(O) \approx 1$, and $w_S^{eff} \approx w_S^{aer}$ indicating fully aerobic yield. Likewise, when $O \ll K_O$, $f(O) \approx 0$, and $w_S^{eff} \approx w_S^{ana}$, indicating a fully anaerobic yield. 
